...
Although not default, =bibtex-actions= also provides convenience functions for auto-refreshing cache when bib files change using filenotify. The simplest use of this functionality is

#+BEGIN_SRC emacs-lisp
(bibtex-actions-filenotify-setup '(LaTeX-mode-hook org-mode-hook))
#+END_SRC
This will add watches for the global bib files and in addition add a hook =LaTeX-mode-hook= and =org-mode-hook= to add watches for local bibliographic files.
By default this will invalidate the cache if the a bib file changes. If the bib files changes rarely, a more suitable option is to refresh the cache. This can be achieved by
#+BEGIN_SRC emacs-lisp
(setq bibtex-actions-filenotify-callback 'refresh-cache)
#+END_SRC
The behavior can be tweaked more thoroughly by setting =bibtex-actions-filenotify-callback= to function. See its documentation for details.
Watches can be also placed on additional files. This is controlled by the variable =bibtex-actions-filenotify-files=.
