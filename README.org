...
This will reload the cache whenever a bib file changes. A different option which might be useful is to merely invalidate the cache so that it is regenerated whenever a
...
(add-hook 'LaTeX-mode-hook (lambda () (bibtex-actions-with-filenotify-local (lambda () (setq bibtex-actions--local-candidates-cache 'uninitialized)))))
(add-hook 'org-mode-hook (lambda () (bibtex-actions-with-filenotify-local (lambda () (setq bibtex-actions--local-candidates-cache 'uninitialized)))))
#+END_SRC
