...
This will reload the cache whenever a bib file changes. A different option which might be useful to merely invalidate the cache so that it s regenerated whenever a
bibtex-actions is called next. This can be achieved by the following code,
