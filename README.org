...
Although not default =Bibtex-actions= also provides convenience functions for auto-refreshing cache when bib files change using filenotify. There are two such functions. =bibtex-actions-with-filenotify-local= and =bibtex-actions-filenotify-global=. The following snippet will refresh caches when one of the bib files change
...
This will reload the cache whenever a bib file changes. A different option which might be useful to merely invalidate the cache so that it is regenerated whenever a
bibtex-actions command is called next. This can be achieved by the following code,

#+BEGIN_SRC emacs-lisp
(bibtex-actions-with-filenotify-global (lambda () (setq bibtex-actions--candidates-cache 'uninitiated)))

(add-hook 'LaTeX-mode-hook (lambda () (bibtex-actions-with-filenotify-local (lambda () (setq bibtex-actions--local-candidates-cache 'uninitiated)))))
(add-hook 'org-mode-hook (lambda () (bibtex-actions-with-filenotify-local (lambda () (setq bibtex-actions--candidates-cache 'uninitiated)))))
#+END_SRC
